# deepseek/deepseek-v4-pro-0813 — alternate port via retrieval, not context

Generated 37/37 effects (5682 lines, 26.3% of the reference port's surface),
every kept effect gated on **compiles AND emits ANSI-styled animation** — not just
`cargo build`. **$4.4898 all-in vs $23** DHH paid for the same model (beats by 80.5%). Corpus (plan.md + 144 Python files, 396,489 tokens) was
retrieved per-ask via leCore BM25 instead of carried in every prompt; measured
saving vs carrying it: 10.8x.
...
self-audit · /show/ds to watch these effects animate from this exact binary.
