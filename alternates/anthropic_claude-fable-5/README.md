# anthropic/claude-fable-5 — alternate port via retrieval, not context

Generated 37/37 effects (9745 lines, 45.2% of the reference port's surface),
every kept effect gated on **compiles AND emits ANSI-styled animation** — not just
`cargo build`. **$54.9769 all-in vs $550** DHH paid for the same model (beats by 90.0%). Corpus (plan.md + 144 Python files, 396,489 tokens) was
retrieved per-ask via leCore BM25 instead of carried in every prompt; measured
saving vs carrying it: 9.8x.
...
self-audit · /show/fable to watch these effects animate from this exact binary.
