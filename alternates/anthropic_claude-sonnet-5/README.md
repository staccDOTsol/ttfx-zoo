# anthropic/claude-sonnet-5 — alternate port via retrieval, not context

Generated 37/37 effects (6673 lines, 30.9% of the reference port's surface),
every kept effect gated on **compiles AND emits ANSI-styled animation** — not just
`cargo build`. **$17.2118 all-in** (DHH did not run this model). Corpus (plan.md + 144 Python files, 396,489 tokens) was
retrieved per-ask via leCore BM25 instead of carried in every prompt; measured
saving vs carrying it: 4.3x.
...
self-audit · /show/sonnet to watch these effects animate from this exact binary.
